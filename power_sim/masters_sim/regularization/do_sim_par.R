...
do_sim_parallel <- function(spectra_list, n_cpus) {
  mclapply(spectra_list, function(spectrum) { 
    tests_results = tryCatch({
      ...
      calculate_hdx_power(noisy_curves,
                          tests = list(deuteros, S_lasso_ridge, S_lasso_ridge_random, memhdx_model),
    ...
    saveRDS(tests_results, file = paste("./results_parallel/", seq, "power.RDS", sep = "_"))
  }, mc.cores = n_cpus)
}
